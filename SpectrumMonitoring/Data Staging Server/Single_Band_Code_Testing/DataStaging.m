...
% Last Modified by GUIDE v2.5 17-Feb-2015 13:49:29
...
if (isfield(handles,'closeFigure') && handles.closeFigure) % Determine whether or not the GUI needs to be closed
    ...
    if ~isempty(popup)
        set(handles.popupmenu1,'String',popup);
        set(handles.popupmenu1,'Value', 1);
    else
        set(handles.popupmenu1,'String','No Available Sensors');
    ...
    timedChecks(hObject,handles); guidata(handles.figure1, handles);
...
for iD=1: len
    ...
    if direct_count ~= 0
        for dL=1: direct_count %Validation Loop
            ...
            try
                vFile = loadjson(filename); [~, qc] = parValidate(filename, vFile, d_list(dL).name); %Validate Files
            catch
                qc = 1;
...
files_obj = timer(...
    ...
    'Period',           600.0, ...                           % updates every xx seconds
    'TasksToExecute',   inf, ...
    'BusyMode',         'drop');
...
try
    ...
    if (isfield(handles,'closeFigure') && handles.closeFigure)
        ...
        [ tDStructure ] = tempcalculations(pDStructure,handles);
        changegui(tDStructure, handles); changeplot(tDStructure, handles);
        set(handles.edit18, 'string', datestr(clock + [0 0 0 0 10 0] , 'HH:MM PM'));
        
        %Saves Values to Figure Structure
        handles.pop = popup; handles.sensor = tDStructure; %Saves Values to Structure
...
%datestr(now)
strlen = length(data); d = datestr(clock);
localD = control.local; vf = 1; 
persistent sF sMF sMG a cal couple cmf cmg NZ
persistent dF dD lF w0r aa con wPk fox sw0r swnPk swT ol
...
for dC=1: strlen
        ...
        switch vFLen
            ...
            case 2
                ...
                if ~isempty(w0r)
                    ...
                    con(aa) = d; wPk(:,aa) = w; fox(aa,:) = x'; sw0r(aa,:) = w0r';
                    swnPk(aa,:) = wnPk'; swT(aa,:) = wT'; ol(aa) = dF.OL;
                end
    ...
    NZ = nnz(ol) + NZ;
    data(dC).files = couple; data(dC).syscal = {cal, cmf, cmg}; data(dC).datacon = {con wPk fox sw0r swnPk swT NZ};
...
index = get(control.popupmenu1,'Value');

try
   filesystem = data(index).files;
catch
    filesystem = '';
end
...
if ~isempty(filesystem)
    try
        sysfile = filesystem{1}; datafile = filesystem{2}; locfile = filesystem{3};
        syscalval = data(index).syscal; dataconval = data(index).datacon;
    catch
        sysfile = filesystem{1}; datafile = filesystem{2}; locfile = '';
        syscalval = data(index).syscal; dataconval = data(index).datacon;
    end
    ...
    set(control.edit7, 'String', data(index).directory_count);
