    ...
    try:
        ...
        if existingAccount == None:
            ...
            return Accounts.packageReturn([
                "INVALUSER",
                "Your email does not match an existing user account. Please contact the system administrator."
            ])
        else:
            ...
            if not retVal[0] == "OK":
                ...
                if tempPasswordRecord == None:
                    ...
                    tempPasswordRecord = {ACCOUNT_EMAIL_ADDRESS: emailAddress,
                    ...
                    retval = [
                        "OK",
                        "You have been sent an email with a web link. Please click on the link to reset your password."
                    ]
                    util.debugPrint("OK")
                    t = threading.Thread(target=generateResetPasswordEmail,
                                         args=(emailAddress, urlPrefix, token))
                    t.daemon = True
                    t.start()
                    return Accounts.packageReturn(retval)
                else:
                    ...
                    return Accounts.packageReturn([
                        "PENDING",
                        "You already have a pending request to reset your password. Please check your email."
                    ])

    except:
        retval = [
            "NOK",
            "There was an issue sending you an email to reset your password. Please contact the system administrator."
        ]
        print "NOK"
        return Accounts.packageReturn(retval)
    finally:
        ...
        if tempPassword == None:
            ...
            if existingAccount == None:
                ...
                DbCollections.getAccounts().update(
                    {"_id": existingAccount["_id"]}, {"$set": existingAccount},
                    upsert=False)
                util.debugPrint("Resetting account password")
                DbCollections.getTempPasswords().remove(
                    {"_id": tempPassword["_id"]})
                return True
    except:
        return False
    finally:
