...
def _getThreshold(jsonData):
    sensor = Sensor(DbCollections.getSensors().find_one(
        {SENSOR_ID: Message.getSensorId(jsonData)}))
        ...
        if threshold[THRESHOLD_SYS_TO_DETECT] == sys2Detect and \
           ...
           threshold["active"]:
            actualThreshold = threshold[
                THRESHOLD_DBM_PER_HZ] + 10 * math.log10(getResolutionBandwidth(jsonData))
...
def getResolutionBandwidth(jsonData):
    if getMeasurementType(jsonData) == FFT_POWER:
        return int((getFmax(jsonData) - getFmin(jsonData)) /
                   getNumberOfFrequencyBins(jsonData))
    else:
        sysMessage = DbCollections.getSystemMessages().find_one(
            {SENSOR_ID: getSensorId(jsonData)})
