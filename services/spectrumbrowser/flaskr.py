...
def getLastSystemMessage(sensorId,sessionId):
   ...
   HTTP Return Codes:
    ...
    @testcase
    def getLastSystemMessageWorker(sensorId,sessionId):
        try:
            if not Config.isConfigured():
                util.debugPrint("Please configure system")
                abort(500)
            if not authentication.checkSessionId(sessionId, USER):
                abort(403)
            return jsonify(GetSystemMessages.getLastSystemMessage(sensorId))
        except:
            print "Unexpected error:", sys.exc_info()[0]
            print sys.exc_info()
            traceback.print_exc()
...
def getDailyMaxMinMeanStats(sensorId, lat, lon, alt, startTime, dayCount, sys2detect, fmin,
    ...
    - sys2detect: the system to detect (e.g. LTE)
   ...
   ::
        ...
        curl -X POST http://localhost:8000/spectrumbrowser/getDailyMaxMinMeanStats/ECR16W4XS/37/-44/55/1404907200/5/745960000/756040000/guest-123
...
def generateSingleAcquisitionSpectrogram(sensorId, startTime, sys2detect,
    ...
    Note that here we do not need coordinates. FFT-Power readings are aggregated.
    All FFT-Power readings belonging to an acquisition are at the same
...
def generateSpectrum(sensorId, start, timeOffset, sessionId):
    ...
    Note that we do not need location parameters here.
