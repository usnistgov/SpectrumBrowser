...
def generateResetPasswordEmail(emailAddress,url):
    """
...
def storePasswordEmailUser(emailAddress,newPassword,url):
    ...
    try:
        t = threading.Thread(target=generateResetPasswordEmail,args=(emailAddress,url))
        t.daemon = True
        t.start()
    except:
        retval = {"status": "NOK"}
