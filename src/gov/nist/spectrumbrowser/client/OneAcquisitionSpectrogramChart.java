...
import gov.nist.spectrumbrowser.client.SpectrogramFragment.Region;
import gov.nist.spectrumbrowser.client.SpectrogramFragment.SurfaceMouseMoveHandlerImpl;
...
public class OneAcquisitionSpectrogramChart implements
	...
	public class SurfaceMouseMoveHandlerImpl implements MouseMoveHandler {
		...
		public void onMouseMove(MouseMoveEvent event) {
			...
			currentFreq = (long) ((maxFreq - minFreq) * yratio)
					+ minFreq;
			currentTime = (long) ((maxTime - minTime) * xratio)
					+ minTime;
			currentValue.setText("Time = "
					+ currentTime + " seconds; Freq = "
					+ currentFreq + " MHz");
		}
	...
	public OneAcquisitionSpectrogramChart(String sensorId, long selectionTime,
		...
		mSpectrumBrowser.getSpectrumBrowserService().generateSingleAcquisitionSpectrogram(
	...
	public void onSuccess(String result) {
		try {
			...
			String colorBarFile = jsonValue.isObject().get("cbar").isString()
					.stringValue();
			spectrogramUrl = SpectrumBrowser.getGeneratedDataPath() + spectrogramFile;
			...
			minFreq = (int)jsonValue.isObject().get("minFreq").isNumber().doubleValue()/1000;
			maxFreq = (int)jsonValue.isObject().get("maxFreq").isNumber().doubleValue()/1000;
			localTimeOfAcquisition = (int)jsonValue.isObject().get("tStartLocalTime").isNumber().doubleValue();
			Date newValue = new Date(localTimeOfAcquisition*1000);
			DateTimeFormat dateTimeFormat = DateTimeFormat
					.getFormat("yyyy-MM-dd HH:mm:ss");
			String timeZone = jsonValue.isObject().get("timeZone").isString().stringValue();
			localDateOfAcquisition = dateTimeFormat.format(newValue) + " " + timeZone;
			minTime = 0;
	...
	private void drawNavigation() {
		...
		vpanel.add(menuBar);
		HTML title = new HTML("<H1>Acquistion Start : " + localDateOfAcquisition + " </H1>");
		vpanel.add(title);
	...
	public void handleSpectrogramLoadEvent() {
		...
		surface.getContext2d().rect(0, 0, canvasPixelWidth, canvasPixelHeight);
		spectrogramPanel
