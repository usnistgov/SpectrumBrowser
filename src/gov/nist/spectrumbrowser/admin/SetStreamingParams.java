...
public class SetStreamingParams {
	...
	public void draw() {
		...
		streamingCaptureIntervalTextBox.setText
		...
		streamingCaptureIntervalTextBox.addValueChangeHandler(new ValueChangeHandler<String> () {
			...
			public void onValueChange(ValueChangeEvent<String> event) {
				try {
					...
					if (!sensorStreamingParams.setStreamingCaptureSamplingIntervalSeconds(streamingCaptureInterval)) {
		...
		grid.setText(row, 0, "Spectrogram Capture Sample Size (sec.)");
		grid.setWidget(row, 1, streamingCaptureSampleSizeTextBox);
		streamingCaptureSampleSizeTextBox.setTitle("The time interval of the captured spectrogram.");
		
		row++;
		final TextBox streamingSecondsPerFrameTextBox = new TextBox();
		streamingSecondsPerFrameTextBox.setText(Float.toString(sensorStreamingParams.getStreamingSecondsPerFrame()));
		...
		grid.setText(row,0 , "Time Resolution Per Frame (sec.)");
		grid.setWidget(row,1,streamingSecondsPerFrameTextBox);
		streamingSecondsPerFrameTextBox.setTitle("Size of the aggregation window frame periodically sent to the browser");
