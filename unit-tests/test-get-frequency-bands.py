...
class TestGetFrequencyBands(unittest.TestCase):
    def setUp(self):
    ...
    def testGetFrequencyBands(self):
        r = requests.post(
            self.url + "/spectrumbrowser/getFrequencyBands/" + self.sessionToken ,
            verify=False)
        print "status code ", r.status_code
        self.assertTrue(r.status_code == 200)
	print r.json()
        freqBands = r.json()["freqBands"]
        found = False
        self.assertTrue(len(freqBands) != 0)
        for freqBand in freqBands:
           if freqBand["systemToDetect"] == "Radar-SPN43":
              found = True
